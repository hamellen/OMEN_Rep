...
void APlayerable_char::player_hited(float input_damage)//플레이어 피격시 
{
	UE_LOG(LogTemp, Warning, TEXT("player_hited"));
	player_hp -= input_damage;//플레이어 체력 감소
	if (player_hp > 0) {//체력 ui 반영
		auto widget = Cast<UPlayer_widget>(first_ui);
...
void APlayerable_char::player_recovery()//플레이어 체력 회복 
{
...
void APlayerable_char::check_bone()//퀘스트 완료 조건이 되는지 상시 검사 
{
...
void APlayerable_char::check_flash()//f키를 활용한 플래쉬라이트 켜기 
{
	is_flash = !is_flash;
